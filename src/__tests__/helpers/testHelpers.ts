...
import { loadTestConfig, getAbapDestination, getServiceKeysDir, getSessionsDir } from '../helpers/configHelpers';
...
/**
 * Get test service keys directory from YAML config
 * Throws error if not configured in YAML
 */
function getTestServiceKeysDir(): string {
  ...
  throw new Error(
    'Service keys directory not configured in tests/test-config.yaml.\n' +
    'Please set auth_broker.paths.service_keys_dir in test-config.yaml'
  );
}

/**
 * Get test sessions directory from YAML config
 * Throws error if not configured in YAML
 */
function getTestSessionsDir(): string {
  ...
  throw new Error(
    'Sessions directory not configured in tests/test-config.yaml.\n' +
    'Please set auth_broker.paths.sessions_dir in test-config.yaml'
  );
}

// Export service keys directory (from YAML config)
 ...
 * Uses YAML configuration for testDestinationsBroker
 */
export function setupTestBrokers(testName: string): TestBrokers {
  const { AuthBrokerTestHelper } = require('./AuthBrokerTestHelper');
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), `auth-broker-${testName}-test-`));
  
  // Create broker with temp directory (for isolated tests)
  // Use 'none' browser to prevent browser opening for isolated tests
  ...
  // Use YAML configuration for test destinations broker
  // Use 'system' browser for tests that need browser auth (Test 2)
  const testDestinationsBroker = AuthBrokerTestHelper.createAbapBrokerFromYaml({
    browser: 'system',
    logger: testLogger
  });
