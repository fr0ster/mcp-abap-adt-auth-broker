  ...
  describe('getServiceKey', () => {
    it('should load valid XSUAA service key file', async () => {
      ...
      if (loaded && typeof loaded === 'object') {
        // getServiceKey returns IConfig (IAuthorizationConfig & IConnectionConfig)
        const config = loaded as { uaaUrl?: string; uaaClientId?: string; uaaClientSecret?: string };
        expect(config.uaaUrl).toBe('https://example.authentication.eu10.hana.ondemand.com');
        expect(config.uaaClientId).toBe('sb-example-12345!t123');
        expect(config.uaaClientSecret).toBe('example-secret-key-12345$example-hash=');
      }
    });
    ...
    it('should search in multiple paths', async () => {
      ...
      if (loaded && typeof loaded === 'object') {
        // getServiceKey returns IConfig (IAuthorizationConfig & IConnectionConfig)
        const config = loaded as { uaaUrl?: string };
    ...
    it('should preserve abap.url if present', async () => {
      ...
      if (loaded && typeof loaded === 'object') {
        // getServiceKey returns IConfig (IAuthorizationConfig & IConnectionConfig)
        const config = loaded as { serviceUrl?: string };
        expect(config.serviceUrl).toBe('https://abap.test.com');
      }
    });
  });
