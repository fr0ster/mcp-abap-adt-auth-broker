...
describe('XsuaaSessionStore', () => {
  ...
  describe('loadSession', () => {
    ...
    it('should load valid XSUAA session file', async () => {
      ...
      if (result && typeof result === 'object' && 'mcpUrl' in result) {
        ...
        expect(config.mcpUrl).toBe('https://example.cfapps.eu10-004.hana.ondemand.com');
    ...
    it('should load session without mcpUrl (mcpUrl is optional)', async () => {
      // Create .env file with XSUAA_JWT_TOKEN but no XSUAA_URL (mcpUrl is optional)
      const envFile = path.join(tempDir, 'mcp.env');
      fs.writeFileSync(envFile, 'XSUAA_JWT_TOKEN=token123\n');
      ...
      if (result && typeof result === 'object') {
        ...
        const config = result as { serviceUrl?: string; authorizationToken?: string };
        expect(config.authorizationToken).toBe('token123');
        expect(config.serviceUrl).toBeUndefined(); // serviceUrl (mcpUrl) is optional
      }
    });

    it('should return null if jwtToken is missing', async () => {
      // Create invalid .env file without XSUAA_JWT_TOKEN
      const envFile = path.join(tempDir, 'mcp.env');
      fs.writeFileSync(envFile, 'XSUAA_UAA_URL=https://uaa.test.com\n');

      // loadXsuaaEnvFile returns null if jwtToken is missing
      const result = await store.loadSession('mcp');
      expect(result).toBeNull();
    });
  ...
  describe('saveSession', () => {
    it('should save XSUAA session configuration', async () => {
      ...
      // mcpUrl can be saved as additional variable (not part of CONNECTION_VARS, but can be stored)
      expect(fileContent).toContain('XSUAA_MCP_URL=https://mcp.test.com');
      expect(fileContent).toContain('XSUAA_JWT_TOKEN=token123');
    });
    ...
    it('should throw error if trying to save ABAP session', async () => {
      ...
      await expect(store.saveSession('mcp', config as any)).rejects.toThrow('XsuaaSessionStore can only store XsuaaSessionConfig');
    });
    ...
    it('should work without mcpUrl (mcpUrl is optional)', async () => {
      ...
      const fileContent = fs.readFileSync(envFile, 'utf8');
      expect(fileContent).toContain('XSUAA_JWT_TOKEN=token123');
      // XSUAA_MCP_URL should not be present since mcpUrl is undefined
      expect(fileContent).not.toContain('XSUAA_MCP_URL=');
    });
