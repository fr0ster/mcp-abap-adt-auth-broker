...
import { XsuaaTokenProvider } from '../../providers';
import { loadTestConfig, hasRealConfig, getXsuaaDestinations, getServiceKeysDir, getSessionsDir } from '../helpers/configHelpers';
...
describe('AuthBroker with XSUAA', () => {
  ...
  describe('refreshToken with XSUAA service key', () => {
    it('should work with XSUAA service key that has UAA credentials', async () => {
      ...
      const authConfig = await serviceKeyStore.getAuthorizationConfig(btp_destination);
      const connConfig = await serviceKeyStore.getConnectionConfig(btp_destination);
      ...
      fs.writeFileSync(skFile, JSON.stringify({
        url: authConfig.uaaUrl,
        clientid: authConfig.uaaClientId,
        clientsecret: authConfig.uaaClientSecret,
        abap: connConfig?.serviceUrl ? { url: connConfig.serviceUrl } : undefined,
      }));

      // Mock client credentials auth for XSUAA (no browser needed)
      const getClientCredentialsTokenSpy = jest.spyOn(require('../../auth/clientCredentialsAuth'), 'getTokenWithClientCredentials');
      ...
      if (savedSession && typeof savedSession === 'object') {
        // loadSession returns IConfig with authorizationToken (not jwtToken)
        const session = savedSession as { serviceUrl?: string; authorizationToken?: string };
        expect(session.authorizationToken).toBeTruthy();
        expect(session.authorizationToken?.length).toBeGreaterThan(0);
        // serviceUrl (mcpUrl) is optional - not checked
      }
    ...
    it('should work with XSUAA service key without SAP URL (XSUAA key only has UAA credentials)', async () => {
      ...
      const startBrowserAuthSpy = jest.spyOn(require('../../auth/browserAuth'), 'startBrowserAuth');
  ...
  describe('getConnectionConfig with XSUAA', () => {
    it('should return mcpUrl from XSUAA session if present', async () => {
      ...
      const connConfig = await broker.getConnectionConfig(btp_destination);
      expect(connConfig?.serviceUrl).toBe(testUrl);
    });

    it('should return undefined serviceUrl for XSUAA service key if no session (XSUAA key has no URL)', async () => {
      if (!btp_destination) {
        return;
      }
      ...
      if (process.env.TEST_VERBOSE) {
        console.log(`📁 XSUAA getConnectionConfig: service key: ${serviceKeyPath}`);
      }
      ...
      const skFile = path.join(tempDir, `${btp_destination}.json`);
      fs.writeFileSync(skFile, JSON.stringify({
        url: authConfig.uaaUrl, // UAA URL, not MCP URL
        clientid: authConfig.uaaClientId,
        clientsecret: authConfig.uaaClientSecret,
      ...
      // For XSUAA, getConnectionConfig should return undefined serviceUrl if no session
      // Because XSUAA service key doesn't contain MCP URL (only UAA credentials)
      const connConfig = await broker.getConnectionConfig(btp_destination);
      expect(connConfig?.serviceUrl).toBeUndefined();
    });
  });
