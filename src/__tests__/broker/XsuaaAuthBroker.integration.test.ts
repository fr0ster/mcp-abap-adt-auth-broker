 ...
 * 2. Fill in real values for <TEST_BTP_DESTINATION>, <TEST_MCP_DESTINATION>, <TEST_MCP_URL>
...
describe('AuthBroker with XSUAA (Integration)', () => {
  ...
  if (shouldSkip) {
    ...
    console.log('  2. Fill in real values for <TEST_BTP_DESTINATION>, <TEST_MCP_DESTINATION>, <TEST_MCP_URL>');
    console.log('  3. Place service key file in service-keys directory');
    return;
  }

  const { btp_destination, mcp_url } = getXsuaaDestinations(config);
  ...
  beforeAll(() => {
    ...
    }, 'none'); // Use 'none' browser to prevent browser opening in tests
  });
  ...
  describe('getConnectionConfig with XSUAA', () => {
    it(`should return MCP URL from session (XSUAA key has no URL) (files: ${btp_destination}.json, ${btp_destination}.env)`, async () => {
      if (!btp_destination || !mcp_url) {
        return;
      }
      ...
      if (process.env.TEST_VERBOSE) {
        console.log(`📁 getConnectionConfig XSUAA: service key: ${serviceKeyPath}, session: ${sessionPath}, URL: ${mcp_url}`);
      }
      ...
      // Now getConnectionConfig should return mcpUrl from session
      const connConfig = await broker.getConnectionConfig(btp_destination);
      
      expect(connConfig?.serviceUrl).toBeTruthy();
      expect(typeof connConfig?.serviceUrl).toBe('string');
      expect(connConfig?.serviceUrl).toBe(mcp_url);
  ...
  describe('XSUAA session management', () => {
    it(`should save and load XSUAA session (files: ${btp_destination}.env)`, async () => {
      ...
      if (loaded) {
        expect(loaded.serviceUrl).toBe(mcp_url);
        expect(loaded.authorizationToken).toBeTruthy();
        expect(typeof loaded.authorizationToken).toBe('string');
        // Don't check specific token values - they are implementation details
      }
